...
export async function GET(request: NextRequest) {
  const { data, status, newCookies } = await djangoFetch(request, "/api/fees/structures/");
...
export async function POST(request: NextRequest) {
  ...
  const djangoResponse = await fetch(`${process.env.DJANGO_API_URL}/api/fees/structures/`, {
    method: "POST",
